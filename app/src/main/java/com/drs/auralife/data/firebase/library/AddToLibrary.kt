    ...
    fun showAddLibraryDialog(context: Context, film: Movie) {
        LibraryRepository.getLibrary { libraries ->
            ...
            libraries.forEach { library ->
                container.addView(AppCompatButton(context).apply {
    ...
    fun showCreateLibraryDialog(context: Context, film: Movie) {
        ...
        btnCreate.setOnClickListener {
            ...
            if (newLibraryName.isNotBlank()) {
                addToLibrary(context, film, newLibraryName)
                dialog.dismiss()
            }
            else {
                Toast.makeText(
            ...
            it.onSuccess {
                Toast.makeText(
                    context, context.getString(
                        if (it) R.string.added_to_library_successfully
                        else R.string.library_already_exists
                    ), Toast.LENGTH_SHORT
                ).show()
            }.onFailure {
                Toast.makeText(context, R.string.failed_to_add_to_library, Toast.LENGTH_SHORT)
                    .show()
            }
        }
    }
